...
for example, in Davenport~\cite[Ch.~16]{davenport} or Iwaniec–Kowalski~\cite{iwaniec-kowalski}.
...
\]
uniformly in $x\in\mathbb{R}$ and $H>0$; see Montgomery–Vaughan~\cite{mv-book}, or
the original short–interval inequalities~\cite{montgomery-vaughan}. In our setting, after freezing slowly varying
...
\end{equation}
where $C_0(\sigma,\alpha;q)$ is explicit and depends on $q$ only through
$\log(qT)$; for $q=1$ it reduces to the constant $\tfrac{1}{2\pi}+o(1)$ of
Appendix~\ref{app:A} (the global second-moment diagonal~\eqref{eq:diag-main}).
