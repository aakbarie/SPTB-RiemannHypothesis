...
Since $F_\lambda(H_\sigma;\,\cdot\,)\ge0$, Landau's theorem identifies the abscissa
of convergence of $\int_0^\infty e^{-uT}F_\lambda\,dT$ with a real singularity,
...
\begin{remark}[A Gram-matrix formulation, after Knill--Laflamme]\label{rmk:gram}
The rigidity admits a linear-algebraic reformulation in the spirit of the
Knill--Laflamme quantum error-correction conditions~\cite{knill-laflamme}, where
correctability of error operators $\{E_a\}$ on a code with projector $P$ is
governed by the \emph{nondegeneracy} of the Hermitian Gram matrix
$c_{ab}=PE_a^\dagger E_bP$: diagonalizing $c$ splits the errors, by polar
decomposition, into an orthogonal (correctable) part and a null (vanishing) part.
Here the analogues of the errors are the edge contributions
$e_\rho(t)=e^{(\eta_\rho+i\gamma_\rho)t}$ (with $\eta_\rho$ near $\eta^\ast$), and
cancellation is exactly degeneracy of their Gram matrix
$G(T)_{\rho\rho'}=\int_0^T e_\rho(t)\,\overline{e_{\rho'}(t)}\,dt$.  A direct
...
weighted, finite-$T$ analogue of Ingham's inequality~\cite{ingham-ineq}).  This
ties the unattained-edge rigidity to the same \emph{spacing} of ordinates that
underlies Hypothesis~(S) of Appendix~\ref{app:A}: both open inputs assert that
the zeros do not cluster.  We do not establish the Schur bound
