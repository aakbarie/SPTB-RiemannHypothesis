...
Assume first $|\gamma|\Delta\ge1$, the oscillation hypothesis of
Lemma~\ref{lem:derivative-penalty}; this holds for every zero with
$|\gamma|\gtrsim\log T$, and in particular for the injected ordinate of Part~3.
By Lemma~\ref{lem:derivative-penalty} applied to $h_\rho(t)=|\rho|^{-\alpha}e^{\eta t}\cos(\gamma t)$
on $I_j$, combined with the blockwise derivative--variance constant $c_{\mathrm{der}}=\tfrac1{12}$
...
For the complementary low-lying case $|\gamma|\Delta<1$ the identical geometric growth
$\gg e^{2\eta t_j}\min\{\Delta,1/|\gamma|\}$ is furnished instead by the amplitude
residual~\eqref{eq:affine-energy} of Step~\ref{step:affine-energy}
(Lemma~\ref{lem:affine-lb}, Cases~1--2), which requires no oscillation hypothesis;
the remainder of the assembly is unchanged.
...
The proved directions leave open the following converse:
...
\noindent
This conjecture is \emph{not proved} in this paper.
Its status is discussed heuristically in Part~4 via geometric analogies.
