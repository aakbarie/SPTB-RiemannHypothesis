...
\begin{remark}[Status]
Sections~\ref{sec:horocycle}–\ref{sec:info-geometry} are heuristic.
...
\begin{remark}[Terminology: this is not the modular horocycle flow]
Our use of ``horocycle'' refers to the critical level set $u=1$ of the
auxiliary metric~\eqref{eq:metric} below, and is a geometric analogy only.
It is \emph{not} the closed-horocycle flow on the modular surface
$\mathrm{SL}_2(\mathbb{Z})\backslash\mathbb{H}$, whose equidistribution rate is
itself equivalent to RH (Zagier~\cite{zagier}, Sarnak~\cite{sarnak-horocycle});
we draw no formal connection to that theory.
\end{remark}
...
This geometric form mirrors the analytic Conjecture~\ref{conj:horocycle}, whose
finite and attained-edge cases are in fact \emph{theorems}
(Propositions~\ref{prop:finite-converse}, \ref{prop:attained-edge}); only the
unattained-edge case remains open, as the rigidity statement of
Remarks~\ref{rmk:rigidity}--\ref{rmk:gram}.
...
% ---------------------------------------------------------
\section{Information-Geometry View}\label{sec:info-geometry}

The information-geometric perspective follows Amari--Nagaoka~\cite{amari}.
Formally,
\[
...
\midrule
Connes~\cite{connes} & Operator/trace positivity & SPTB: curvature-bounded energy \\
Berry--Keating~\cite{berry-keating} & $H=xp$ semiclassical ansatz & SPTB: geodesic/energy flow \\
Balazs--V\"or\"os~\cite{balazs-voros} & Periodic-orbit analogy & SPTB: horocycle confinement \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item Quantitatively verified constants ($<3\times10^{-4}$ relative error).  
\end{enumerate}
...
\qquad
\beta>\sigma \Rightarrow F_\lambda\gg e^{2(\beta-\sigma)T}.
}
\]
