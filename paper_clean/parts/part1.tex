...
against the integration length $T$.  Lemma~\ref{lem:MV-sieve} is retained only
for the Dirichlet-polynomial estimates of Appendix~\ref{app:dirichlet}, where the
frequencies $\log n$ are genuinely well spaced.
\end{remark}
...
Theorem~\ref{thm:variance} holds independently of RH.  Its leading term is
unconditional; the error term uses, in place of a large-sieve estimate for the
(clustered) ordinates, only the mild spacing Hypothesis~\ref{hyp:spacing}~(S),
which is far weaker than RH or pair correlation (Appendix~\ref{app:A},
Remark~\ref{rmk:spacing}).  Constants are uniform over the canonical
regime~\eqref{eq:canon}.
