...
\medskip
\noindent\textbf{Non-equivalence disclaimer and surrogate scope.}
The exponential signal $e^{(\beta-\sigma)t}\cos(\gamma t)$ that drives our
detection theorem is not posited: it is the explicit-formula expansion of the
genuine $\zeta$-derived remainder $G_\sigma(t)=e^{-\sigma t}(\psi(e^t)-e^t)$
(Section~\ref{sec:explicit-formula}), in which an off-line zero injects exactly
this term with weight $|\rho|^{-1}$ and a phase $\varphi_\rho$. The quantitative
analysis, however, is carried out on a \emph{tractable square-summable surrogate}
$H_\sigma$ of $G_\sigma$, a deliberate modeling choice that makes the variance
bound clean. Accordingly: (a) the forward direction---off-line zero
$\Rightarrow$ exponential growth---is, for $G_\sigma$, an instance of classical
Tur\'an--Pintz $\Omega$-oscillation read out by $F_\lambda$, not a new forcing
mechanism; (b) the polynomial upper bound holds verbatim for the surrogate
$H_\sigma$ but requires the smoothing window for $G_\sigma$; and (c) the
converse (polynomial boundedness $\Rightarrow$ $\beta\le\sigma$) is unproved (the
Horocycle Conjecture). We therefore present SPTB as a finite-window numerical
diagnostic over known oscillation theory, and claim no equivalence with RH.
...
with real frequencies $\gamma$,
\[
\int_u^{u+\Delta}\!|D(t)|^2\,dt
\;\le\; (\Delta+\delta^{-1})\sum_{|\gamma|\le X}|A_\gamma|^2,
\]
where $\delta$ is the minimal frequency gap.  This applies to both $H_\sigma$ and
$\partial_t H_\sigma$ (see~\cite{montgomery-vaughan} and Lemma~\ref{lem:MV-sieve}).
We caution that for the zeros of $\zeta$ the minimal gap is \emph{not} bounded
below by $1/\log T$ (zeros cluster, cf.\ pair correlation~\cite{montgomery-pair}).
As explained in Remark~\ref{rmk:spacing}, the variance bound does not in fact
rely on (A4) for the zero frequencies: it is obtained directly from the global
second moment in Appendix~\ref{app:A}, with (A4) used only for the well-spaced
Dirichlet-polynomial frequencies $\log n$.
\end{enumerate}
...
The surrogate-level analysis of Parts~1–2 extends to Dirichlet $L(s,\chi)$
(primitive $\chi$; Appendix~\ref{app:dirichlet}) and, conjecturally, to
automorphic $L$-functions of degree~$d$~\cite{selberg} satisfying analogues of
(A1)–(A4): the variance bound relies only on (A2)–(A4), and the bias bound
isolates any single zero with $\beta>\sigma$.  The explicit-formula grounding of
Section~\ref{sec:explicit-formula} is written for $\zeta(s)$; it transfers to
$L(s,\chi)$ through the corresponding explicit formula (with the analytic
conductor replacing $\log T$), and to higher degree only heuristically.
Proofs are stated for $\zeta(s)$ for clarity; Part~3 validates numerically for
$\zeta(s)$, and Appendix~A lists constants.
\end{remark}
...
\begin{remark}[The derivative is a truncated object]\label{rmk:truncation}
Differentiating \eqref{eq:Hsigma} termwise produces coefficients of size
$|\gamma|\,|\rho|^{-\alpha}$, so the series for $\partial_t H_\sigma$ converges
absolutely only for $\alpha>2$.  At the explicit-formula value $\alpha=1$
(Section~\ref{sec:explicit-formula}) it is understood as the \emph{truncated}
field $\partial_t H_\sigma^{(X)}=\sum_{|\gamma|\le X}\partial_t(\,\cdot\,)$ with
the canonical cutoff $X=\Delta^{-1}\asymp\log T$.  Every occurrence of
$\int_{I_j}|\partial_t H_\sigma|^2$ in Parts~1–2 refers to this truncation,
whose short-interval norm is exactly what (A4) and Appendix~\ref{app:A} bound;
the cutoff is the source of the $\log T$ in $N(X)\ll X\log X$ used there.
\end{remark}
...
(Li~\cite{li}, with complements by Bombieri--Lagarias~\cite{bombieri-lagarias};
Lagarias~\cite{lagarias}; the Nyman--Beurling/B\'aez-Duarte criterion~\cite{beurling,baezduarte};
see also the survey~\cite{conrey})
...
\begin{proof}
This is a specialization of the Montgomery--Vaughan large sieve
inequality~\cite{montgomery-vaughan}; see Remark~\ref{rmk:spacing} for the
spacing issue and Appendix~\ref{app:dirichlet} for details.
\end{proof}

\begin{remark}[Spacing is not actually needed]\label{rmk:spacing}
The large sieve constant $\delta^{-1}$ in Lemma~\ref{lem:MV-sieve} uses the
\emph{minimal} gap $\delta$ of the active frequencies, and for the ordinates of
$\zeta$ no positive lower bound on $\delta$ holds uniformly (the
pair-correlation law predicts gaps far below the mean).  We therefore do
\emph{not} prove the variance bound through Lemma~\ref{lem:MV-sieve}.  Instead,
because the blocks tile $[0,T]$, the quantity
$\sum_j\int_{I_j}|H_\sigma'|^2$ equals the \emph{global} second moment
$\int_0^T|H_\sigma'|^2$, which Appendix~\ref{app:A} bounds directly: the diagonal
yields the main term $\tfrac{1}{2\pi}T\log T\log\log T$ \emph{unconditionally},
while the off-diagonal is merely polylogarithmic in $T$ under the mild ordinate
spacing Hypothesis~(S) (Hypothesis~\ref{hyp:spacing}).  Hypothesis~(S) is far
weaker than RH or pair correlation and is the single arithmetic input; crucially,
it replaces --- and is much milder than --- the uniform minimal-gap requirement
of the large sieve, because the truncation height $X\asymp\log T$ is negligible
against the integration length $T$.  Lemma~\ref{lem:MV-sieve} is recorded for
completeness --- it applies to genuinely well-spaced frequencies, such as the
$\log n$ of Dirichlet polynomials, but \emph{not} to the clustered zeta
ordinates, which is why the variance bound is obtained by the global second
moment instead.
\end{remark}
...
Theorem~\ref{thm:variance} holds independently of RH, and is essentially
unconditional: the leading (diagonal) term is unconditional, and the off-diagonal
error term requires, in place of a large-sieve estimate for the clustered
ordinates, only the \emph{trivial} low-height separation
$\delta_{\min}(\log T)\gg T^{-1}(\log T)^{O(1)}$
(Appendix~\ref{app:A}, Remark~\ref{rmk:obstruction}), which holds with enormous
room since the zeros below height $\log T$ have gaps $\asymp1/\log\log T$.  The
clean uniform Hypothesis~\ref{hyp:spacing}~(S) is far more than this bound
needs; the genuinely open use of non-clustering is not here but in the
unattained-edge rigidity of Part~2 (Remarks~\ref{rmk:rigidity}--\ref{rmk:gram}).
Constants are uniform over the canonical regime~\eqref{eq:canon}.

\begin{proof}[Proof of Theorem~\ref{thm:variance}]
Since the blocks tile $[0,T]$ and the blockwise affine projection only decreases
each residual, $F_\lambda$ is controlled by the global second moments of $H_\sigma$
and of its truncated derivative; we do \emph{not} route the bound through the
large sieve for the (clustered) zero frequencies (cf.\ Remark~\ref{rmk:spacing}).
For the slope term, the reduction~\eqref{eq:reduce-global} and the
diagonal/off-diagonal estimate of Appendix~\ref{app:A} give
\begin{equation*}
\sum_j\int_{I_j}|R_j'|^2 \;\le\; C_0'\,T\log T\log\log T
\qquad\text{\eqref{eq:residC0prime}},
\end{equation*}
the leading order coming from the diagonal~\eqref{eq:diag-main}; the off-diagonal
is negligible (polylogarithmic in $T$) using only the mild low-height separation
of Remark~\ref{rmk:obstruction}.  For the amplitude term, the same diagonal
computation without the derivative weight gives
\begin{equation*}
\sum_j\int_{I_j}|R_j|^2 \;\le\; \int_0^T|H_\sigma|^2\,dt
\;\ll\; T\sum_{|\gamma|\le X}|\rho|^{-2\alpha} \;\ll\; T
\end{equation*}
for $\alpha>1/2$ (Proposition~\ref{prop:summability}).  Since
$\lambda\le c_2(\log T)^{-2}\le1$,
\begin{equation*}
F_\lambda(H_\sigma;T,\Delta)
=\sum_j\Big(\int_{I_j}|R_j|^2+\lambda\int_{I_j}|R_j'|^2\Big)
\;\ll\; T + C_0'\,T\log T\log\log T
\;\ll\; T\log T\log\log T. \qedhere
\end{equation*}
\end{proof}
