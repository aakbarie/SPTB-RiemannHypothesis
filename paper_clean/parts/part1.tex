...
the inequality is therefore applied in the averaged form of
Remark~\ref{rmk:spacing}, which is what introduces the $\log\log T$ factor.
\end{enumerate}
...
\begin{remark}[Spacing caveat and intended argument]\label{rmk:spacing}
The large sieve constant $\delta^{-1}$ above uses the \emph{minimal} gap
$\delta$ of the active frequencies.  For the ordinates of $\zeta$ no positive
lower bound on $\delta$ holds (the GUE/pair-correlation law predicts gaps far
below the mean), so Lemma~\ref{lem:MV-sieve} cannot be invoked with
$\delta\asymp1/\log T$ uniformly.  The bound is instead established in mean over
the block grid: grouping zeros by local density and applying a Gallagher-type
short-interval second-moment estimate, the clustered frequencies contribute
$\ll(\Delta+\log X)$ \emph{on average}, with the zero-counting estimate (A2)
supplying the residual $\log\log T$.  This averaged form is what Parts~1–2 use;
we flag it as the technical crux of the variance bound and do not claim the
naive minimal-spacing large sieve.
\end{remark}
