...
\item[(A4)] A Montgomery–Vaughan–type \emph{short-interval inequality}~\cite{montgomery-vaughan}
for the Dirichlet
...
Parts~1–2 extend to Dirichlet $L(s,\chi)$ (primitive $\chi$) and, conjecturally, to
automorphic $L$-functions of degree~$d$ satisfying analogues of (A1)–(A4)
(for which the required zero-density and spacing inputs follow the framework
of Rudnick--Sarnak~\cite{rudnick-sarnak}).
...
Classical equivalence criteria for RH—Li's positivity criterion~\cite{li-criterion}
(with complements by Bombieri and Lagarias~\cite{bombieri-lagarias}), the Lagarias
elementary form~\cite{lagarias}, Speiser's criterion~\cite{speiser}, and the
Nyman--Beurling/B\'aez-Duarte mean-square criterion~\cite{beurling,baezduarte}—involve
global data and infinitely many zeros (see also Conrey's survey~\cite{conrey-notices}).  The SPTB functional is
...
constant $c_{\mathrm{der}}=4\pi^2$, the sharp constant in the affine Poincaré
inequality $\|r'\|_{L^2(I)}^2\ge(c_{\mathrm{der}}/\Delta^2)\|r\|_{L^2(I)}^2$ for
$r\perp\mathrm{span}\{1,t\}$ on unit intervals.
